    ...
    <ol class="prepared-script-list">
      <li v-for="(script, index) in scripts" :key="script.id" :class="{ active: snapshot.activeScriptId === script.id }">
        ...
        <div class="prepared-script-fields">
          <label>Chế độ<select v-model="script.role" @change="emit('changed')"><option value="idle">Chờ</option><option value="activation">Kích hoạt</option><option value="conversation">Đang nói</option></select></label>
