...
const settings = { enabled: true, scripts: [
  { id: 'r1', name: 'R1', enabled: true, order: 0, playbackType: 'video' as const, mediaLayerId: 'video-r1', speechText: '', interruptMode: 'immediate' as const, completionMode: 'next' as const },
  { id: 'r2', name: 'R2', enabled: true, order: 1, playbackType: 'audio' as const, mediaLayerId: 'audio-r2', speechText: '', interruptMode: 'after-current' as const, completionMode: 'stop' as const },
  { id: 'r3', name: 'R3', enabled: true, order: 2, playbackType: 'tts' as const, mediaLayerId: null, speechText: 'Xin chao', interruptMode: 'immediate' as const, completionMode: 'stop' as const },
] };
...
describe('prepared script playback controller', () => {
  it('plays scripts in sequence and ignores stale media callbacks', () => {
    ...
    expect(controller.snapshot()).toMatchObject({ activeScriptId: 'r2', activeLayerId: 'audio-r2' });
  });
